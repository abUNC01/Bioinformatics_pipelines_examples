...
##>  Headers for curl to denote I want the data back in json format
curl -X GET "${API_CALL}" \
  -H 'Accept: application/json' \
  -H 'Content-Type: application/json' \
  -o "molecular_data_${drug_query}.${FILE_OUTPUT_FORMAT}"

#> print to shell what the data return file is
echo "Molecular data for ${1} is save to molecular_data_${1}.${FILE_OUTPUT_FORMAT}" 
